#!/bin/bash
#
# Qwen3-30B-A3B deterministic training (EP8).
#
# Same workload as run-qwen3-30B-A3B.sh (GBS=256, n=8, resp=8192, CPU-offload
# Adam, GRPO), with the aligned Megatron/SGLang stack from
# tests/test_qwen3_30b_a3b_deterministic_e2e.py.
#
# DeepGEMM alignment requires TP=1 / ETP=1, so this is pure EP8 rather than
# the production TP=4 EP=8 layout.
#
#   bash scripts/run-qwen3-30B-A3B-deterministic.sh
#   NUM_ROLLOUT=1 bash scripts/run-qwen3-30B-A3B-deterministic.sh
#   SLIME_E2E_SHAPE=1 NUM_ROLLOUT=1 ENABLE_EVAL=0 bash scripts/run-qwen3-30B-A3B-deterministic.sh
#   ROLLOUT_MAX_RESPONSE_LEN=32 NUM_ROLLOUT=1 ENABLE_EVAL=0 bash scripts/run-qwen3-30B-A3B-deterministic.sh
#
# Env: HF_MODEL PROMPT_DATA EVAL_DATA SAVE LOAD REF_LOAD NUM_ROLLOUT
#      CI_TEST ENABLE_EVAL MAX_TRAIN_ROLLOUT_DIFF SGLANG_KV_CACHE_DTYPE
#      SLIME_E2E_CONTEXT_PARALLEL_SIZE SGLANG_ROOT MEGATRON_ROOT

# rerun cleanup — do not pkill python (shared container)
pkill -9 sglang || true
sleep 3
ray stop --force || true
pkill -9 ray || true
...
# Default docker soft limit is 1024. Ray prestarts one worker per CPU (288 on
# this B200 box) and raylet then dies with "Too many open files", which shows
# up as a gRPC timeout to the node-manager port.
ulimit -n 524288
...
# Architecture only. Do not enable fused MoE permute (e2e / DeepGEMM path).
MODEL_ARGS=(
   --disable-bias-linear
   --qk-layernorm
...
# alignment_env() + Qwen3 overrides (SGLANG_MASKED_GEMM_FAST_ACT=0: expert dim 768)
...
# SLIME_E2E_SHAPE=1: same alignment stack, e2e workload (GBS=8, n=1, resp=32).
...
if [ "${E2E_SHAPE}" = "1" ]; then
  ...
  # Production batch. Override any of these from the environment, e.g.
  # ROLLOUT_MAX_RESPONSE_LEN=32 to keep GBS=256 / n=8 / decode-bs=32.
...
WANDB_ARGS=(
   ...
   --wandb-group qwen3-30B-A3B-deterministic
   --wandb-mode offline
